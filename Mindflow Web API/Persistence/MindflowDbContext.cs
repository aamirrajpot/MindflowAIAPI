...
namespace Mindflow_Web_API.Persistence
{
    public class MindflowDbContext(DbContextOptions<MindflowDbContext> options) : DbContext(options)
    {
        ...
        public DbSet<AiContentReport> AiContentReports => Set<AiContentReport>();
        ...
        protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
        {
            optionsBuilder
                .UseAsyncSeeding(async (context, _, cancellationToken) =>
                {
                    ...
                    if (sampleMovie == null)
                    {
                        sampleMovie = Movie.Create("Sonic the Hedgehog 3", "Fantasy", new DateTime(2025, 1, 3, 0, 0, 0, DateTimeKind.Utc), 7);
